...
- **The forward pass returns device buffers, not `burn::Tensor<4>`.** In the Burn
  revision the fork pins (`0.22.0-pre.2`), `Tensor<const D>` is backend-erased over
  the *fusion* backend, and the only public route from a raw `CubeTensor` back into
  one is a registered fusion `Operation` — the ~90 lines of `BindOp` boilerplate in
  `brush-render/src/burn_glue.rs`, normally generated by `#[backend_extension]` plus
  `#[derive(ExtensionType)]`. `PyramidRender` therefore exposes the composited
  `(P, C)` buffer and its `t_final`/`n_used` companions as
  `CubeTensor<WgpuRuntime>` — which is exactly what a viewer binds anyway (compare
  `brush-render`'s `resolve_to_cube_float`, whose whole purpose is to hand that type
  to a wgpu pipeline). The `Tensor<4>` wrapper is deferred to the task that wires up
  the U-Net and `splat_backbuffer.rs`, since that is what will consume it.
